   ...
   //(task 1)    tests test cases
     ...
     */
     ...
     for(int x=0;x<FSM.length;x++){
    	 try{
    	 Object[] array=(FSM[x].getLabels().toArray());
    	 
    	 
    	 for(int y=0;y<array.length;y++){
    		 String str=array[y].toString();
    		 str=str.substring(0,str.indexOf("/"));
    		 boolean in=false;
    		 for(int z=0;z<types.size();z++){
    			 if(types.get(z).equals(str)){
    				 in=true;
    			 }
     ...
     String modified="";//new 
     for(int x=0;x<tests.size();x++){
    	 String t=tests.get(x);
    	 for(int z=0;z<t.length();z++){
    	 
    		 ArrayList<Integer> index=new ArrayList<Integer>();
    		 
    	 for(int y=0;y<types.size();y++){
    		 String str=t.substring(0, types.get(y).length());
    		 System.out.println(str+"    "+types.get(y)+"       "+str.equals(types.get(y))); 
    		 if(str.equals(types.get(y))){
    			 index.add(y);
    	 ...
    	 if(index.size()>1){
    		 ...
    		 for(int v=0;v<index.size();v++){
    			 if(types.get(index.get(v)).length()>highest){
    				 hi=index.get(v);
    				 highest=types.get(hi).length();
    			 }
    		 }
    	 }
    	 else{
    	 ...
    	 t=t.substring(types.get(index.get(hi)).length()-1,t.length());//might have an error;
    	 modified+=types.get(index.get(hi))+" ";
    	 
    	 index=new ArrayList<Integer>();
    	 }
    	
    	 Utilities.runFSM(FSM, 1, modified, " ");
    	 modified="";
     }
    */
